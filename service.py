...
def openURLAdress(url, postData=None):
    try:
        ...
        if postData:
            ...
            website = urllib.request.urlopen(req + data)
        else:
        ...
        # read html code
        # html = website.read()
        html = website.read().decode('latin-1')
        website.close()
        return html
    ...
    if subtitles_list:
        for it in subtitles_list:
            listitem = xbmcgui.ListItem(label=it["language_name"],
                                        label2=it["filename"])
            listitem.setArt({'icon': "lang"}
        )
                                       # setArt=it[{'icon': "language_flag"}]
                                        # iconImage=it["rating"],
                                        #)
...
def old_extractor(temp_archive, subtitle_list = []):
    ...
    for subfile in xbmcvfs.listdir(temp_archive)[1]:
            ...
            if (os.path.splitext(file)[1] in exts):
                ...
                xbmc.executebuiltin(('Extract("%s","%s")' % (file, __temp__,))
                    .encode('utf-8'), True)
...
def Download(url, filename, language_name=None):
    ...
    try:
        ...
        subFile.close()
        xbmc.sleep(500)
        xbmc.executebuiltin(('Extract("%s","%s")' % (temp_file, __temp__,))
            .encode('utf-8'), True)
...
if params['action'] == 'search':
    ...
    # item['title'] = item['title'].replace(b'[B]', bytes('', 'UTF-8'))
    # item['title'] = item['title'].replace(b'[/B]', bytes('', 'UTF-8'))
    # item['title'] = item['title'].replace(b'  ', bytes(' ', 'UTF-8'))
    ...
    item['title'] = unquoted = urllib.parse.unquote(item['title'])
